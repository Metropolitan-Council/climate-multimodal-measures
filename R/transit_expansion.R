transit_expansion <- function(ridership_increase, route_type, added_transit) {
  
  adjustment_factor <- X
  
  average_trip_length <- TRIP LENGTH BY ROUTE TYPE %>% filter(route_type == route_type)
  ...
  for (i in seq_along(project_years)) {
    ...
    vmt_displaced_year <- ridership_increase * average_trip_length * adjustment_factor
